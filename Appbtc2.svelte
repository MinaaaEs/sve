<script>
  ...
  try {
    ...
    let previousData = timeSeries[previousDate];
    
    let headers = Object.keys(latestData).map(key => key.replace(/^\d+\. /, ''));
    headers.unshift("Date");
    
    let latestRow = [latestDate, ...Object.values(latestData)];
    let previousRow = [previousDate, ...Object.values(previousData)];
    
    columnHeaders = headers;
    tableData = isAfter ? [previousRow] : [latestRow];
  ...
  async function fetchApiData(classType) {
   ...
   else if (classType === 'bitcoin') {  // Handle Bitcoin
    fetchBitcoinData();  // Call your new Bitcoin fetch function
    return;  // Return early as data is handled in fetchBitcoinData
  }
