  ...
  async function fetchApiData(classType) {
    ...
    try {
      ...
      if (apiData) {
        ...
        // Convert apiData values into a 2D array format required for Handsontable
        tableData = [Object.values(apiData)]; // Wrap values inside an array to make it a row

        console.log(`Updated columnHeaders for ${classType}:`, columnHeaders);
        console.log(`Updated tableData for ${classType}:`, tableData);
      } else {
        console.error(`No exchange rate data found for ${classType}`);
      }
    } catch (err) {
      error = err.message;
