...
let highlightedCells = new Set(); // Store highlighted cells
...
function highlightSignificantDifferences(selectedData, referenceData) {
  ...
  tableData = tableData.map((row, rowIndex) => {
    return row.map((cell, colIndex) => {
      if (colIndex === 0 || isNaN(cell)) return cell; // Skip date column (index 0) and non-numeric values
      ...
      if (isNaN(referenceValue)) return cell; // Skip if reference value is not a valid number

      // Calculate the percentage change
      let percentageChange = Math.abs((cell - referenceValue) / referenceValue);
  ...
  // Apply styles for highlighting (this will be done in the table rendering)
  applyHighlighting();
}

function applyHighlighting() {
  // Apply the highlighted cells using the index stored in highlightedCells
  tableData = tableData.map((row, rowIndex) => {
    return row.map((cell, colIndex) => {
      let cellKey = `${rowIndex}-${colIndex}`;
      // Add a class or style to the cell if it's in the highlighted cells set
      if (highlightedCells.has(cellKey)) {
        // Wrap the cell in a span with the highlight class
